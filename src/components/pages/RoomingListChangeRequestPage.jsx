...
import { calculateRoomingListChanges, calculateRoomTypePickupSummary, getRoomingListByToken, reviewRoomingListChangeRequest } from "../../services/firebaseRoomingLists";
...
export default function RoomingListChangeRequestPage() {
  ...
  const pickupSummary = calculateRoomTypePickupSummary(roomingList?.roomTypeDays || roomingList?.group?.roomTypeDays, base, request?.reservations || []);
...
function RoomTypePickupOverview({ summary }) {
  const pickupChange = summary.totals.requestedPickedUp - summary.totals.officialPickedUp;
  return <Card>
    <div><h2 className="text-lg font-semibold">Room Types by Day</h2><p className="mt-1 text-sm text-gray-600">Pickup in the requested list compared with the current official version. Remaining is the number of blocked room nights still available to pick up.</p></div>
    <div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
      <Metric label="Total Blocked" value={summary.totals.blocked} />
      <Metric label="Official Picked Up" value={summary.totals.officialPickedUp} />
      <Metric label="Requested Picked Up" value={summary.totals.requestedPickedUp} detail={`${pickupChange >= 0 ? "+" : ""}${pickupChange} change`} tone={pickupChange === 0 ? "neutral" : "changed"} />
      <Metric label="Still Available" value={summary.totals.remaining} />
    </div>
    <div className="mt-5 overflow-x-auto pb-2"><div className="grid auto-cols-[minmax(18rem,1fr)] grid-flow-col gap-4">
      {summary.days.map((day) => <section key={day.date} className="rounded-xl border border-gray-200 bg-gray-50 p-4">
        <div className="flex items-start justify-between gap-3"><div><h3 className="font-semibold text-gray-900">{new Date(`${day.date}T00:00:00`).toLocaleDateString(undefined, { weekday: "short", month: "short", day: "numeric" })}</h3><p className="text-xs text-gray-500">{day.date}</p></div><span className="rounded-full bg-white px-2.5 py-1 text-xs font-semibold text-gray-700 shadow-sm">{day.remaining} remaining</span></div>
        <div className="mt-3 space-y-2">{day.roomTypes.map((roomType) => <div key={`${day.date}-${roomType.code}`} className={`rounded-lg border p-3 text-sm shadow-sm ${roomType.pickupChange === 0 ? "border-gray-100 bg-white" : "border-amber-200 bg-amber-50"}`}>
          <div className="flex justify-between gap-3"><p className="font-semibold">{roomType.code} <span className="font-normal text-gray-500">{roomType.name}</span></p>{roomType.pickupChange !== 0 && <span className="font-semibold text-amber-800">{roomType.pickupChange > 0 ? "+" : ""}{roomType.pickupChange}</span>}</div>
          <div className="mt-2 grid grid-cols-3 gap-2 text-xs text-gray-600"><p>Blocked<br /><b className="text-gray-900">{roomType.blocked}</b></p><p>Picked up<br /><b className="text-gray-900">{roomType.requestedPickedUp}</b></p><p>Remaining<br /><b className="text-gray-900">{roomType.remaining}</b></p></div>
          {roomType.pickupChange !== 0 && <p className="mt-2 text-xs text-amber-800">Official pickup: {roomType.officialPickedUp} → Requested: {roomType.requestedPickedUp}</p>}
        </div>)}</div>
        <div className="mt-3 flex justify-between border-t border-gray-200 pt-3 text-xs font-semibold text-gray-700"><span>{day.requestedPickedUp} picked up</span><span>{day.blocked} blocked</span></div>
      </section>)}
    </div></div>
  </Card>;
}
...
function Metric({ label, value, detail, tone = "neutral" }) {
  return <div className={`rounded-xl border p-4 ${tone === "changed" ? "border-amber-200 bg-amber-50" : "border-gray-200 bg-gray-50"}`}><p className="text-xs font-semibold uppercase tracking-wide text-gray-500">{label}</p><p className="mt-1 text-2xl font-semibold text-gray-900">{value}</p>{detail && <p className="mt-1 text-xs font-semibold text-amber-800">{detail}</p>}</div>;
}
