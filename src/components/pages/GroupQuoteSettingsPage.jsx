...
export default function GroupQuoteSettingsPage() {
  ...
  const handleSubmit = async (event) => {
    ...
    setSaving(true);
    await saveGroupQuoteSettings(hotelUid, { inflationPercentage: Number(inflationPercentage) });
    setSaving(false);
  ...
  return <div className="min-h-screen bg-gray-50 text-gray-900"><HeaderBar today={today} onLogout={handleLogout} /><PageContainer className="space-y-6">
    ...
    <Card>{loading ? <p>Loading settings...</p> : <form onSubmit={handleSubmit} className="space-y-5"><label className="block max-w-md text-sm font-semibold">Inflation %<input required min="0" step="0.01" type="number" value={inflationPercentage} onChange={(event) => setInflationPercentage(event.target.value)} className="mt-2 w-full rounded-lg border border-gray-300 px-3 py-2 font-normal" /></label><div><button disabled={saving} className="rounded-lg bg-[#b41f1f] px-5 py-2 font-semibold text-white disabled:bg-gray-400">{saving ? "Saving..." : "Save Settings"}</button></div></form>}</Card>
  </PageContainer></div>;
}
