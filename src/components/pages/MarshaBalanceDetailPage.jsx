...
function RuleResult({ calculation }) {
  if (calculation.applicable === false) return <span className="font-semibold text-gray-500">Not applicable</span>;
...
export default function MarshaBalanceDetailPage() {
  ...
  const mappingColumns = [
    ...
    { key: "values", label: "Values", sortable: false, render: (calculation) => calculation.applicable === false ? "Condition not met" : calculation.assessable ? <div><span>{calculation.rawMarshaValue} MARSHA / {calculation.rawOperaValue} Opera{calculation.reservedRooms ? ` − ${calculation.reservedRooms} reserved` : ""}</span>{(calculation.rawMarshaValue !== calculation.marshaValue || calculation.rawOperaValue !== calculation.operaValue) && <span className="block text-xs text-gray-500">Compared as {calculation.marshaValue} / {calculation.operaValue}; negative values count as zero</span>}</div> : "Required value missing" },
    { key: "difference", label: "Difference", render: (calculation) => calculation.applicable !== false && calculation.assessable ? <strong>{calculation.difference}</strong> : "—" },
  ...
  return <div className="min-h-screen bg-gray-50 text-gray-900">
    ...
    <PageContainer className="space-y-6">
      ...
      {!loading && detail && <>
        ...
        {totalCalculations.map((calculation) => <Card key={calculation.rule.id} className="p-4"><div className="mb-3 flex items-center justify-between gap-3"><div><h2 className="font-semibold">All physical room totals</h2><p className="text-xs text-gray-500">Total control, separate from room mappings</p></div><RuleResult calculation={calculation} /></div>{calculation.applicable === false ? <p className="text-sm text-gray-500">The configured remaining-room condition is not met for this date.</p> : calculation.assessable ? <div className="grid gap-3 sm:grid-cols-3"><ValueBox label="MARSHA total" value={calculation.marshaValue} /><ValueBox label="Opera total after reservations" value={calculation.comparedOperaValue} /><ValueBox label="Difference" value={calculation.difference} result={calculation.within} /></div> : <p className="text-red-700">The totals cannot be assessed.</p>}</Card>)}
        <div><h2 className="mb-3 text-xl font-semibold">Room type mappings</h2><DataListTable columns={mappingColumns} rows={mappingCalculations} emptyMessage="No active room type mappings configured." getRowProps={(calculation) => ({ className: calculation.applicable === false ? "bg-gray-50" : calculation.assessable && calculation.within ? "bg-green-50" : "bg-red-50" })} /></div>
