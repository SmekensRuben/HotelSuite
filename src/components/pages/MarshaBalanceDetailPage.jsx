...
export default function MarshaBalanceDetailPage() {
  ...
  const availabilityColumns = [
    { key: "marshaCode", label: "MARSHA type", render: (row) => <strong>{row.marshaCode}</strong> },
    { key: "marshaValue", label: "Availability" },
    { key: "effectiveMinimum", label: "Effective minimum", render: (row) => row.effectiveMinimum ?? "—" },
    { key: "excluded", label: "Excluded Opera rooms", sortable: false, render: (row) => row.excludedOperaTypes?.length ? row.excludedOperaTypes.map((item) => <div key={item.code}>{item.code}: {item.value ?? "missing"}</div>) : "None" },
    { key: "suitableInventory", label: "Suitable inventory", render: (row) => row.suitableInventory ?? "—" },
    { key: "applied", label: "Applied rules", sortable: false, render: (row) => row.appliedOperaRules.map((item) => <div key={item.operaType}>{item.operaType}: {item.applied.counts ? "counts" : "excluded"} ({item.applied.source}: {item.applied.description})</div>) },
  ];
  ...
  return <div className="min-h-screen bg-gray-50 text-gray-900"><HeaderBar today={new Date().toLocaleDateString("en-GB", { timeZone: "Europe/Brussels", weekday: "long", month: "long", day: "numeric" })} onLogout={logout} /><PageContainer className="space-y-6">
    ...
    {!loading && detail && <><Card className={`border ${detail.code === "ok" ? "border-green-200" : ["warning", "review", "unconfigured"].includes(detail.code) ? "border-amber-200" : "border-red-200"}`}><Result value={detail} /><p className="mt-2 text-sm text-gray-600">{detail.alerts.length ? `${detail.alerts.length} finding(s) for this stay date.` : "All configured controls passed."}</p></Card>
      ...
      {detail.marshaDoc && <Card><h2 className="text-lg font-semibold">Explicit hotel totals</h2><dl className="mt-3 grid gap-3 sm:grid-cols-2">{[["MARSHA total", detail.marshaTotal], ["Opera total", detail.operaTotal]].map(([label, value]) => <div key={label} className="rounded bg-gray-50 p-3"><dt className="text-xs text-gray-500">{label}</dt><dd className="text-xl font-semibold">{value ?? "Missing"}</dd></div>)}</dl></Card>}
      <div><h2 className="mb-3 text-xl font-semibold">MARSHA availability rules</h2><DataListTable columns={availabilityColumns} rows={(detail.availabilityAssessments || []).map((item) => ({ ...item, id: item.marshaCode }))} emptyMessage="No MARSHA availability rules configured." /></div>
