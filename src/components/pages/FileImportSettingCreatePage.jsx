...
import { createFileImportSetting } from "../../services/firebaseSettings";
...
export default function FileImportSettingCreatePage() {
  ...
  const handleSubmit = async (event) => {
    ...
    if (!hotelUid || !formValues.reportName.trim()) return;

    setSaving(true);
    const created = await createFileImportSetting(hotelUid, {
      ...formValues,
    ...
    if (created?.id) {
      navigate(`/settings/file-import/${created.id}`);
      return;
    }
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900">
      ...
      <PageContainer className="space-y-6">
        <div>
          ...
          <h1 className="text-3xl font-semibold">Create File Import Setting</h1>
        </div>
        ...
        <Card>
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div className="flex justify-end gap-2">
              <button
                type="button"
                onClick={() => navigate("/settings/file-import")}
