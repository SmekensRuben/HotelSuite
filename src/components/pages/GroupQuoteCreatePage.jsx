  ...
  return <div className="min-h-screen bg-gray-50 text-gray-900">
    ...
    <PageContainer className="space-y-6 pb-10">
      ...
      {analysisQuote && <section className="space-y-6">
        ...
        {contribution && !contribution.validationError && <details className="rounded-lg border border-gray-200 bg-white shadow-sm"><summary className="cursor-pointer p-4 text-lg font-semibold">How is the Economic Floor calculated?</summary><div className="grid gap-5 border-t p-5 lg:grid-cols-3">
          ...
          <section><p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Stage 2 · Required Net Room Revenue</p><dl className="mt-4 space-y-2 text-sm">{[["Total opportunity cost", contribution.totalLostContribution, ""], ["Group variable room costs", contribution.groupVariableRoomCosts, "+"], ["Group breakfast costs", contribution.groupBreakfastCosts, "+"], ["BQT contribution", contribution.bqtContribution, "−"]].map(([label, value, sign]) => <div key={label} className="flex justify-between gap-4"><dt>{sign} {label}</dt><dd>{value === null ? "Unavailable" : currency(value)}</dd></div>)}<div className="flex justify-between gap-4 border-t pt-2 font-semibold"><dt>Required revenue after commission</dt><dd>{contribution.requiredNetGroupRoomRevenue === null ? "Unavailable" : currency(contribution.requiredNetGroupRoomRevenue)}</dd></div></dl></section>
          <section><p className="text-xs font-semibold uppercase tracking-wide text-gray-500">Stage 3 · Commission / Floor</p><dl className="mt-4 space-y-2 text-sm"><div className="flex justify-between gap-4"><dt>Required revenue after commission</dt><dd>{contribution.requiredNetGroupRoomRevenue === null ? "Unavailable" : currency(contribution.requiredNetGroupRoomRevenue)}</dd></div><div className="flex justify-between gap-4"><dt>Group commission</dt><dd>{percentage(contribution.groupCommission)}</dd></div><div className="flex justify-between gap-4"><dt>Required gross room revenue</dt><dd>{contribution.requiredGrossGroupRoomRevenue === null ? "Unavailable" : currency(contribution.requiredGrossGroupRoomRevenue)}</dd></div><div className="flex justify-between gap-4"><dt>Requested room nights</dt><dd>{rooms(contribution.totalRequestedGroupRoomNights)}</dd></div><div className="flex justify-between gap-4 border-t pt-2 font-semibold"><dt>Economic Floor Rate</dt><dd>{contribution.economicFloorRate === null ? "Unavailable" : currency(contribution.economicFloorRate)}</dd></div></dl></section>
        </div></details>}
