    ...
    <PageContainer className="space-y-6 pb-10">
      ...
      {analysisQuote && <section className="space-y-6">
        ...
        {contribution?.validationError ? <Card className="border border-red-200 bg-red-50 text-red-800"><strong>Contribution settings error:</strong> {contribution.validationError}</Card> : contribution && <>
          ...
          <PricingGuidance guidance={pricingGuidanceSnapshot} ownPublicRateInclVat={marketContextSnapshot?.groupStaySummary?.weightedOwnPublicRateInclVat ?? null} />
        ...
        {contribution && !contribution.validationError && <details className="rounded-lg border border-gray-200 bg-white shadow-sm"><summary className="cursor-pointer p-4 text-lg font-semibold">Transient Value</summary><div className="space-y-4 border-t p-5">
          <p className="text-sm text-gray-600">True transient room value uses deductible transient room revenue from the same dates selected by the Transient Demand Forecast. Rates are excl. VAT unless noted.</p>
          {contribution.nightly.map((night) => <section key={night.stayDate} className="rounded-lg border border-gray-200 p-4"><h4 className="font-semibold">{night.stayDate}</h4><dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
            {[["Historical Comparable Transient ADR", night.historicalComparableTransientAdrExVat === null ? "Unavailable" : currency(night.historicalComparableTransientAdrExVat)], ["Current Transient OTB ADR", night.currentTransientOtbAdrExVat === null ? "Unavailable" : currency(night.currentTransientOtbAdrExVat)], ["Expected Future Transient ADR", night.expectedFutureTransientRoomRateExVat === null ? "Unavailable" : currency(night.expectedFutureTransientRoomRateExVat)], ["Historical ADR observations", night.historicalTransientAdrEvidenceCount], ["Distribution Cost", percentage(Number(quoteSettings.transientDistributionCostPercentage || 0) / 100)], ["Room Contribution / RN", night.transientRoomContributionPerRoom === null ? "Unavailable" : currency(night.transientRoomContributionPerRoom)], ["Breakfast Contribution / RN", currency(night.transientBreakfastContributionPerRoom)], ["Total Contribution / RN", night.futureTransientContributionPerRoom === null ? "Unavailable" : currency(night.futureTransientContributionPerRoom)], ["Value Confidence", night.transientValueConfidence || "Unavailable"]].map(([label, value]) => <div key={label}><dt className="text-gray-500">{label}</dt><dd className="font-medium">{value}</dd></div>)}
          </dl>{night.scenarios.base.displacedFutureTransientRooms > 0 && <p className="mt-3 rounded bg-gray-50 p-3 text-sm"><strong>Base lost future transient contribution:</strong> {rooms(night.scenarios.base.displacedFutureTransientRooms)} RN × {currency(night.futureTransientContributionPerRoom)} = {currency(night.scenarios.base.lostFutureTransientContribution)}</p>}</section>)}
        </div></details>}
        {contribution && !contribution.validationError && <details className="rounded-lg border border-gray-200 bg-white shadow-sm"><summary className="cursor-pointer p-4 text-lg font-semibold">Future Group Value</summary><div className="space-y-4 border-t p-5">
          <p className="text-sm text-gray-600">Future Group Value uses one excl.-VAT rate observation per selected historical Group Forecast comparable plus an optional current deductible existing-group signal. Pipeline commercial context is displayed separately because its VAT/package basis is unknown.</p>
          {contribution.nightly.map((night) => <section key={night.stayDate} className="rounded-lg border border-gray-200 p-4"><h4 className="font-semibold">{night.stayDate}</h4><dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2 lg:grid-cols-4">
            {[['Historical Comparable Group ADR', night.historicalComparableGroupAdrExVat === null ? 'Unavailable' : currency(night.historicalComparableGroupAdrExVat)], ['Current Existing Group ADR', night.currentExistingGroupAdrExVat === null ? 'Unavailable' : currency(night.currentExistingGroupAdrExVat)], ['Expected Future Group ADR', night.expectedFutureGroupRoomRateExVat === null ? 'Unavailable' : currency(night.expectedFutureGroupRoomRateExVat)], ['Group-demand comparable dates', night.groupDemandComparableCount], ['Historical rate observations', night.historicalAdrEvidenceCount], ['Future Group Commission', percentage(night.expectedFutureGroupCommission)], ['Future Group Contribution / RN', night.futureGroupContributionPerRoom === null ? 'Unavailable' : currency(night.futureGroupContributionPerRoom)], ['Value Confidence', night.futureGroupValueConfidence || 'Unavailable'], ['Evidence source', night.futureGroupValueSource]].map(([label, value]) => <div key={label}><dt className="text-gray-500">{label}</dt><dd className="font-medium">{value}</dd></div>)}
          </dl>{night.hasPipelineCommercialRate && <div className="mt-4 rounded-lg border border-blue-100 bg-blue-50 p-3 text-sm"><p><span className="text-gray-600">Prospect Pipeline Commercial Rate</span> <strong className="ml-2">{currency(night.pipelineCommercialRate)}</strong></p><p className="mt-1 text-xs text-gray-600">Informational only. This non-deductible pipeline value may include VAT and breakfast/package components and is not used in Future Group Contribution.</p></div>}{night.scenarios.base.displacedFutureGroupRooms > 0 && <p className="mt-3 rounded bg-gray-50 p-3 text-sm"><strong>Base lost future group contribution:</strong> {rooms(night.scenarios.base.displacedFutureGroupRooms)} RN × {currency(night.futureGroupContributionPerRoom)} = {currency(night.scenarios.base.lostFutureGroupContribution)}</p>}
          {night.historicalGroupAdrObservations.length > 0 && <div className="mt-4 overflow-x-auto"><table className="min-w-full text-xs"><thead><tr className="border-b text-left text-gray-500">{['Date', 'Group Rooms', 'Deductible Group Revenue', 'Raw ADR', 'Inflation-adjusted ADR'].map((heading) => <th key={heading} className="p-2">{heading}</th>)}</tr></thead><tbody>{night.historicalGroupAdrObservations.map((item) => <tr key={item.date} className="border-b"><td className="p-2">{item.date}</td><td className="p-2">{rooms(item.groupRooms)}</td><td className="p-2">{currency(item.groupRevenueDeductible)}</td><td className="p-2">{currency(item.rawGroupAdrExVat)}</td><td className="p-2">{currency(item.inflationAdjustedGroupAdrExVat)}</td></tr>)}</tbody></table></div>}
          </section>)}
        </div></details>}
