        ...
        {contribution?.validationError ? <Card className="border border-red-200 bg-red-50 text-red-800"><strong>Contribution settings error:</strong> {contribution.validationError}</Card> : contribution && <>
          <Card className="overflow-hidden border border-gray-200 bg-white p-0 shadow-sm">
            ...
            <div className="grid gap-6 p-6 lg:grid-cols-[minmax(0,1fr)_minmax(20rem,0.9fr)]">
              <div>
                <div className="rounded-xl border-2 border-blue-200 bg-blue-50 p-5"><p className="text-sm font-bold uppercase tracking-wide text-blue-800">Economic Floor Rate</p><p className="mt-1 text-4xl font-bold text-gray-900">{contribution.economicFloorRate === null ? "Unavailable" : currency(contribution.economicFloorRate)}</p><p className="mt-1 text-sm font-medium text-gray-600">Base group-demand scenario</p><p className="mt-4 text-sm text-gray-700">The adjusted Economic Floor protects expected future transient and group contribution. Future group demand is based on Group Forecast V1.</p><dl className="mt-4 grid grid-cols-2 gap-3 text-sm sm:grid-cols-3"><div><dt className="text-gray-600">Transient-only Floor</dt><dd className="font-semibold">{contribution.transientOnlyEconomicFloor === null ? "Unavailable" : currency(contribution.transientOnlyEconomicFloor)}</dd></div><div><dt className="text-gray-600">Low Group Demand Floor</dt><dd className="font-semibold">{contribution.economicFloorLow === null ? "Unavailable" : currency(contribution.economicFloorLow)}</dd></div><div><dt className="text-gray-600">High Group Demand Floor</dt><dd className="font-semibold">{contribution.economicFloorHigh === null ? "Unavailable" : currency(contribution.economicFloorHigh)}</dd></div></dl></div>
          ...
          <Card className="border border-gray-200 bg-white shadow-sm"><div><p className="text-xs font-semibold uppercase tracking-widest text-gray-500">Contribution Bridge</p><h3 className="mt-1 text-lg font-semibold">Contribution breakdown</h3><p className="mt-1 text-sm text-gray-600">Base group-demand scenario.</p></div><dl className="mt-5 divide-y divide-gray-100">{[["Lost future transient contribution", contribution.totalLostFutureTransientContribution === null ? "Unavailable" : currency(contribution.totalLostFutureTransientContribution), "text-gray-900"], ["Lost future group contribution", contribution.totalLostFutureGroupContribution === null ? "Unavailable" : currency(contribution.totalLostFutureGroupContribution), "text-gray-900"], ["Total lost contribution", contribution.totalLostContribution === null ? "Unavailable" : currency(contribution.totalLostContribution), "text-gray-900"], ["Group variable room costs", `-${currency(contribution.groupVariableRoomCosts)}`, "text-red-700"], ["Group breakfast costs", `-${currency(contribution.groupBreakfastCosts)}`, "text-red-700"], ["BQT contribution", `+${currency(contribution.bqtContribution)}`, "text-green-700"], ["Group commission", percentage(contribution.groupCommission), "text-gray-900"]].map(([label, value, tone]) => <div key={label} className="flex items-center justify-between gap-4 py-3 first:pt-0 last:pb-0"><dt className="text-sm font-medium text-gray-600">{label}</dt><dd className={`text-base font-semibold tabular-nums ${tone}`}>{value}</dd></div>)}</dl></Card>
        ...
        <Card className="border border-gray-200 bg-white shadow-sm">
          ...
          <div className="space-y-3">{Object.values(forecastData?.byDate || {}).map((result) => <details key={result.stayDate} className={`rounded-lg border ${result.forecastConfidence === "low" ? "border-amber-300 bg-amber-50" : "border-gray-200"}`}>
            ...
            <div className="border-t border-inherit p-4"><dl className="grid gap-x-5 gap-y-2 text-sm sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div><dt className="text-gray-500">Prospect / non-deductible group rooms</dt><dd className="font-medium">{rooms(result.groupProspectPipelineRooms)} rooms</dd><p className="text-xs text-gray-500">Pipeline/prospect business; not treated as committed inventory in this displacement calculation.</p></div>
              <div><dt className="text-gray-500">Future group rate proxy</dt><dd className="font-medium">{Number.isFinite(contribution?.nightly?.find((night) => night.stayDate === result.stayDate)?.expectedFutureGroupRoomRate) ? currency(contribution.nightly.find((night) => night.stayDate === result.stayDate).expectedFutureGroupRoomRate) : "Unavailable"}</dd><p className="text-xs text-gray-500">{contribution?.nightly?.find((night) => night.stayDate === result.stayDate)?.futureGroupRateSource === "PROSPECT_PIPELINE_ADR" ? "Prospect pipeline ADR" : "Existing deductible group ADR"}</p></div>
              <div><dt className="text-gray-500">Hard other committed rooms</dt><dd className="font-medium">{rooms(result.hardOtherCommittedRooms)} rooms</dd></div>
              ...
              <div><dt className="text-gray-500">Not displacing transient demand</dt><dd className="font-semibold text-green-700">{rooms(result.notDisplacingTransientDemand)} rooms</dd></div>
            </dl>
