...
export default function GroupQuoteCreatePage() {
  ...
  return <div className="min-h-screen bg-gray-50 text-gray-900"><HeaderBar today={today} onLogout={handleLogout} /><PageContainer className="space-y-6 pb-10">
    ...
    {analysisQuote && <section className="space-y-4"><div><h2 className="text-2xl font-semibold">Analysis overview</h2><p className="text-sm text-gray-600">Comparable available dates with the same day of week for each selected year.</p></div>{analysis.length ? analysis.map(({ year, matches }) => <Card key={year}><div className="mb-3 flex flex-wrap items-center justify-between gap-3"><h3 className="text-lg font-semibold">{year}</h3><div className="flex items-center gap-2"><button type="button" onClick={() => setWeekOffsets((current) => ({ ...current, [year]: Number(current[year] || 0) - 1 }))} className="rounded border border-gray-300 p-1.5 hover:bg-gray-100" aria-label={`Move ${year} one week back`}><ChevronLeft className="h-4 w-4" /></button><span className="min-w-24 text-center text-xs font-medium text-gray-600">{weekOffsets[year] ? `${weekOffsets[year] > 0 ? "+" : ""}${weekOffsets[year]} week${Math.abs(weekOffsets[year]) === 1 ? "" : "s"}` : "Same week"}</span><button type="button" onClick={() => setWeekOffsets((current) => ({ ...current, [year]: Number(current[year] || 0) + 1 }))} className="rounded border border-gray-300 p-1.5 hover:bg-gray-100" aria-label={`Move ${year} one week forward`}><ChevronRight className="h-4 w-4" /></button></div></div><div className="overflow-x-auto"><table className="min-w-full text-sm"><thead className="border-b text-left text-xs uppercase text-gray-500"><tr><th className="py-2 pr-4">Day</th><th className="px-4 py-2">Considered Date</th><th className="px-4 py-2">Inventory Rooms</th><th className="px-4 py-2">Occupied Rooms</th><th className="px-4 py-2">Average Room Rate</th><th className="px-4 py-2">Adjusted Average Room Rate</th><th className="px-4 py-2">Displaced Rooms</th><th className="py-2 pl-4">Displaced Revenue</th></tr></thead><tbody className="divide-y divide-gray-200">{matches.map((match) => <tr key={match.quoteDate}><td className="py-2 pr-4 font-semibold">{match.weekday}</td><td className="px-4 py-2">{match.consideredDate?.date || match.historicalDate || "-"}</td><td className="px-4 py-2">{Number(match.consideredDate?.calculatedInventoryRooms || 0)}</td><td className="px-4 py-2">{Number(match.consideredDate?.calculatedOccRooms || 0)}</td><td className="px-4 py-2">€{Number(match.consideredDate?.averageRoomRate || 0).toFixed(2)}</td><td className="px-4 py-2">€{match.adjustedAverageRoomRate.toFixed(2)}</td><td className="px-4 py-2">{match.displacedRooms}</td><td className="py-2 pl-4">€{match.displacedRevenue.toFixed(2)}</td></tr>)}</tbody></table></div></Card>) : <Card><p className="text-gray-600">Select at least one historical year to view the analysis.</p></Card>}{analysis.length > 0 && <Card><h3 className="mb-4 text-xl font-semibold">Analysis Summary</h3><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{analysisSummary.displacementByYear.map((item) => <div key={item.year} className="rounded-lg bg-gray-50 p-3"><p className="text-xs uppercase text-gray-500">Total displacement {item.year}</p><p className="text-lg font-semibold">€{item.totalDisplacement.toFixed(2)}</p></div>)}<div className="rounded-lg bg-red-50 p-3"><p className="text-xs uppercase text-gray-500">Average displacement</p><p className="text-lg font-semibold">€{analysisSummary.averageDisplacement.toFixed(2)}</p></div><div className="rounded-lg bg-green-50 p-3"><p className="text-xs uppercase text-gray-500">Profitable from</p><p className="text-lg font-semibold text-green-800">€{analysisSummary.profitablePrice.toFixed(2)} per room</p></div></div></Card>}<div className="flex justify-end"><button type="button" disabled={saving} onClick={handleSave} className="rounded-lg bg-[#b41f1f] px-5 py-2 font-semibold text-white disabled:bg-gray-400">{saving ? "Saving quote..." : "Save Quote"}</button></div></section>}
  </PageContainer></div>;
}
