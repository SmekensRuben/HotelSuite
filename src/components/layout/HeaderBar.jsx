...
import { ChartColumnIncreasing, FileText, Package, Settings2, ShoppingBasket, Truck, Users } from "lucide-react";
...
export default function HeaderBar({ today, onLogout }) {
  ...
  const revenueManagementMenuItems = [
    {
      label: "Pick-Up",
      action: () => navigate("/revenue-management/pick-up"),
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      ...
      if (
        revenueManagementMenuRef.current &&
        !revenueManagementMenuRef.current.contains(event.target)
      ) {
